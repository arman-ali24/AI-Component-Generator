...
const Home = () => {
  ...
  const ai = new GoogleGenAI({
    apiKey: "AIzaSyCFGtEFIBd2-3RGDt63w6pcnznwA43PzcU"
  });
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 px-4 md:px-10 lg:px-16 mt-5">
        ...
        <div className="overflow-hidden w-full h-[80vh] py-6 rounded-xl bg-[#141319] p-5 flex flex-col">
          ...
          <h3 className='text-[23px] md:text-[26px] font-semibold sp-text'>AI Component Generator</h3>
          ...
          <div className="flex items-center justify-between mt-3">
            <p className='text-gray-400 text-sm'>Click generate button to get your code</p>
            ...
            !outputScreen ? (
              ...
              <>
                <div className="bg-[#17171C] w-full h-[50px] flex items-center gap-3 px-3">
                  <button onClick={() => setTab(1)} className={`w-1/2 py-2 rounded-lg ${tab === 1 ? "bg-purple-600" : "bg-zinc-800"} text-white`}>
                    Code
                  </button>
                  <button onClick={() => setTab(2)} className={`w-1/2 py-2 rounded-lg ${tab === 2 ? "bg-purple-600" : "bg-zinc-800"} text-white`}>
                    Preview
                  </button>
                </div>

                <div className="bg-[#17171C] w-full h-[50px] flex items-center justify-between px-4">
                  <p className='font-bold text-gray-200 text-sm md:text-base'>Code Editor</p>
                  <div className="flex items-center gap-2">
                    {tab === 1 ? (
                      <>
                        <button onClick={copyCode} className="w-10 h-10 rounded-xl border border-zinc-800 flex items-center justify-center"><IoCopy /></button>
                        <button onClick={downnloadFile} className="w-10 h-10 rounded-xl border border-zinc-800 flex items-center justify-center"><PiExportBold /></button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => setIsNewTabOpen(true)} className="w-10 h-10 rounded-xl border border-zinc-800 flex items-center justify-center"><ImNewTab /></button>
                        <button onClick={() => setRefreshKey(prev => prev + 1)} className="w-10 h-10 rounded-xl border border-zinc-800 flex items-center justify-center"><FiRefreshCcw /></button>
                      </>
                    )}
                  </div>
                </div>
      ...
      {isNewTabOpen && (
        <div className="absolute inset-0 bg-white w-screen h-screen overflow-auto">
          <div className="text-black w-full h-[60px] flex items-center justify-between px-5 bg-gray-100">
            ...
            <button onClick={() => setIsNewTabOpen(false)} className="w-10 h-10 rounded-xl border border-zinc-300 flex items-center justify-center"><IoCloseSharp /></button>
          </div>
